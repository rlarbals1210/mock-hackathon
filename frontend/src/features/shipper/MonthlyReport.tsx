      ...
      {showReport && (
        <div className="modal-backdrop report-modal-backdrop" role="presentation">
          <section aria-labelledby="report-dialog-title" aria-modal="true" className="report-dialog" role="dialog">
            ...
            <section><h3>조정 결과의 근거</h3><ul><li>후보 수: 기획자료의 3시간 111명, 48시간 1,766명 끝점</li><li>운임·배차시간: 기획안 42만원·40분 ↔ 38만원·15분 끝점</li><li>탄소: 국내계수판 방식 B, 12,000건 가상 오더</li><li>유찰 확률: 원자료에 산정 규칙이 없어 수치 미표시</li></ul></section>
            <section><h3>최근 운영 기록</h3><table><thead><tr><th>일시</th><th>작업</th><th>내용</th></tr></thead><tbody>{operations.slice(0, 6).map((operation) => <tr key={operation.id}><td>{operation.time}</td><td>{operation.title}</td><td>{operation.detail}</td></tr>)}</tbody></table></section>
